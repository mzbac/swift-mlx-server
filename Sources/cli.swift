...
func createCompletionChunkResponse(completionId: String, requestedModel: String, nextChunk: String) -> CompletionChunkResponse {
    return CompletionChunkResponse(completionId: completionId, requestedModel: requestedModel, nextChunk: nextChunk)
}
...
func routes(_ app: Application, _ modelPath: String) async throws {
    ...
    app.post("v1", "completions") { req async throws ->  Response in
        ...
        if stream {
            let headers = HTTPHeaders([("Content-Type", "text/event-stream"), ("Cache-Control", "no-cache")])
                    ...
                    do {
                        ...
                        for try await token in stream_res {
                            ...
                            if stopSequenceBuffer.count > maxStopIdSequenceLen {
                                ...
                                let stopCondition = stoppingCriteria(tokens: tokens, stopIdSequences: stopIdSequences, eosTokenId: eosTokenId)
                                ...
                                let nextChunk = String(generatedText[generatedText.index(generatedText.startIndex, offsetBy: currentGeneratedTextIndex)...])
                                currentGeneratedTextIndex = generatedText.count
                                let chunkResponse = createCompletionChunkResponse(completionId: "cmpl-\(UUID().uuidString)", requestedModel: requestedModel, nextChunk: nextChunk)
            ...
            let completionChoice = CompletionChoice(text: completionText, index: 0, logprobs: nil, finishReason: "stop")
            let completionUsage = CompletionUsage(promptTokens: promptTokens.count, completionTokens: generatedTokens.count, totalTokens: promptTokens.count + generatedTokens.count)
            
            let completionResponse = CompletionResponse(id: "cmpl-\(UUID().uuidString)", object: "text_completion", created: Int(Date().timeIntervalSince1970), model: requestedModel, choices: [completionChoice], usage: completionUsage)
